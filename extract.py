...
def extract_invoice(pages, media_type: Optional[str] = None,
                    ...
                    text: Optional[str] = None) -> InvoiceData:
    ...
    if text is not None:
        content = [{"type": "text", "text": text},
                   {"type": "text", "text":
                    "Extract this supplier invoice from the email text above. Capture EVERY "
                    "line item exactly, and use the order/invoice total as total_ex_gst "
                    "(compute ex-GST = inclusive / 1.1 if only a GST-inclusive total is shown)."}]
    else:
